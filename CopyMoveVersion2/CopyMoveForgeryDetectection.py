...
img=cv2.imread(r"images\trees.jpg")
...
for ClassInd,conf,boxes in zip(ClassIndex.flatten(),confidence.flatten(),bbox):
    if f'{ClassInd}' not in object_boxes_dict.keys():
        object_boxes_dict[f'{ClassInd}']=[boxes]
    else:
        value_list=object_boxes_dict[f'{ClassInd}']
        value_list.extend([boxes])
        object_boxes_dict[f'{ClassInd}']=value_list

print(object_boxes_dict.values())
...
for items in object_boxes_dict:
    Object_Boxes=object_boxes_dict[items]
    print(items)
    print(Object_Boxes)
